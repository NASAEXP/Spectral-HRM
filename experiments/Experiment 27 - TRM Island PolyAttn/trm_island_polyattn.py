...
from models.layers import Attention, FLAGatedDeltaNetAttention, FourierLinear, PolyAttention, PoMAttention, SLAAttention
from models.lm_head import DenseTiedVocab, LMHead, TiedFourierVocab
from models.transformer import TRMIslandBlock
...
DEFAULT_VARIANTS = "spectral-dense-tied-deep,spectral-trm-island-attention,spectral-trm-island-polyattn"
VARIANT_SPECS = {
    "dense-attention": {
        "base_variant": "dense",
        "L_mixer": "attention",
    ...
    "spectral-trm-island-attention": {
        ...
        "L_mixer": "pom",
        "H_mixer": "fla_gdn",
        "island_mixer": "attention",
    },
    "spectral-trm-island-polyattn": {
        "base_variant": "fourier-all-dense-tied-vocab",
        "L_mixer": "pom",
        "H_mixer": "fla_gdn",
        "island_mixer": "polyattn",
    },
}
...
def make_config(*,
                ...
                pom_order: int,
                n_layers: int,
                trm_island_every: int,
                trm_island_steps: int) -> dict:
    ...
    if "island_mixer" in spec:
        h_override["trm_island_every"] = trm_island_every
        h_override["trm_island_mixer"] = spec["island_mixer"]
        h_override["trm_island_steps"] = trm_island_steps
...
def count_modules(model: torch.nn.Module) -> dict[str, int]:
    return {
        ...
        "polyattn_modules": sum(1 for module in model.modules() if isinstance(module, PolyAttention)),
        ...
        "trm_island_modules": sum(1 for module in model.modules() if isinstance(module, TRMIslandBlock)),
...
def train_once(*,
    ...
    config = make_config(
        ...
        pom_order=pom_order,
        n_layers=n_layers,
        trm_island_every=trm_island_every,
        trm_island_steps=trm_island_steps,
    )
    ...
    for step in range(steps):
        offset = (1 + step) * numseqs * total_len
    ...
    return {
        ...
        "island_mixer": spec.get("island_mixer", "none"),
...
def format_row(row: dict[str, float | int | str]) -> str:
    return (
        f"{row['variant']} seed={row['seed']} base={row['base_variant']} L={row['L_mixer']} H={row['H_mixer']} island={row['island_mixer']}: "
        ...
        f"trm_islands={int(float(row['trm_island_modules']))}, "
        f"polyattn={int(float(row['polyattn_modules']))}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare sparse TRM-style island blocks with PolyAttn inside the H-level only.")
    ...
    parser.add_argument("--trm-island-every", type=int, default=4)
    parser.add_argument("--trm-island-steps", type=int, default=2)
    ...
    print(f"n_layers={args.n_layers}, trm_island_every={args.trm_island_every}, trm_island_steps={args.trm_island_steps}")
    ...
    for seed in seeds:
        for variant in variants:
            row = train_once(
                ...
                trm_island_every=args.trm_island_every,
                trm_island_steps=args.trm_island_steps,
