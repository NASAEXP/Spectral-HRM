...
from models.layers import Attention, DeltaNetAttention, PoMAttention, SLAAttention
...
DEFAULT_VARIANTS = "pom-sla,pom-deltanet,pom-precond-deltanet"
VARIANT_SPECS = {
    "pom-sla": {
        "L_mixer": "pom",
        "H_mixer": "sla",
    ...
    "pom-precond-deltanet": {
        "L_mixer": "pom",
        "H_mixer": "precond_deltanet",
    },
}
...
def count_mixers(model: torch.nn.Module) -> dict[str, int]:
    return {
        ...
        "deltanet_modules": sum(1 for module in model.modules() if isinstance(module, DeltaNetAttention) and not module.preconditioned),
        "precond_deltanet_modules": sum(1 for module in model.modules() if isinstance(module, DeltaNetAttention) and module.preconditioned),
    }
...
def train_once(*,
    ...
    return {
        ...
        "attention_modules": float(mixers["attention_modules"]),
        "pom_modules": float(mixers["pom_modules"]),
        "sla_modules": float(mixers["sla_modules"]),
        "deltanet_modules": float(mixers["deltanet_modules"]),
        "precond_deltanet_modules": float(mixers["precond_deltanet_modules"]),
...
def format_row(row: dict[str, float | int | str]) -> str:
    return (
        ...
        f"deltanet={int(float(row['deltanet_modules']))}, "
        f"precond_deltanet={int(float(row['precond_deltanet_modules']))}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare DeltaNet-family H-level candidates against SLA.")
