...
from models.layers import Attention, DeltaNetAttention, GatedDeltaNetAttention, PoMAttention, SLAAttention
...
DEFAULT_VARIANTS = "pom-sla,pom-deltanet,pom-gdn"
VARIANT_SPECS = {
    "pom-sla": {
        "L_mixer": "pom",
        "H_mixer": "sla",
...
def count_mixers(model: torch.nn.Module) -> dict[str, int]:
    return {
        ...
        "deltanet_modules": sum(
            1
            for module in model.modules()
            if isinstance(module, DeltaNetAttention)
            and not isinstance(module, GatedDeltaNetAttention)
            and not module.preconditioned
        ),
        "precond_deltanet_modules": sum(
            1
            for module in model.modules()
            if isinstance(module, DeltaNetAttention)
            and not isinstance(module, GatedDeltaNetAttention)
            and module.preconditioned
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare Gated DeltaNet H-level against SLA and basic DeltaNet.")
