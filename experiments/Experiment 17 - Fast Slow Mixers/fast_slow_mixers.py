...
from models.layers import Attention, PoMAttention, SpectreAttention
...
DEFAULT_VARIANTS = "attention-attention,attention-spectre,pom-attention,pom-spectre"
VARIANT_SPECS = {
    "attention-attention": {
        "L_mixer": "attention",
        "H_mixer": "attention",
...
def count_mixers(model: torch.nn.Module) -> dict[str, int]:
    return {
        ...
        "spectre_modules": sum(1 for module in model.modules() if isinstance(module, SpectreAttention)),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare fast L-level PoM and slow H-level SPECTRE mixers.")
