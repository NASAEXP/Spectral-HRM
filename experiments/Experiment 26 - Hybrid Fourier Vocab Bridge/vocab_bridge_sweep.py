...
from models.lm_head import DenseTiedVocab, HybridFourierLowRankVocab, LMHead, TiedFourierVocab
...
DEFAULT_VARIANTS = "dense-attention,dense-tied-attention,spectral-tied-fourier,spectral-hybrid-r16,spectral-hybrid-r64,spectral-hybrid-r128,spectral-dense-tied"
VARIANT_SPECS = {
    "dense-attention": {
        "base_variant": "dense",
        "L_mixer": "attention",
...
def count_modules(model: torch.nn.Module) -> dict[str, int]:
    return {
        ...
        "tied_fourier_vocab_modules": sum(1 for module in model.modules() if isinstance(module, TiedFourierVocab) and not isinstance(module, HybridFourierLowRankVocab)),
        "hybrid_vocab_modules": sum(1 for module in model.modules() if isinstance(module, HybridFourierLowRankVocab)),
    }
...
def train_once(*,
    ...
    for module in model.modules():
        if isinstance(module, (TiedFourierVocab, HybridFourierLowRankVocab)):
    ...
    return {
        ...
        "residual_rank": float(spec.get("residual_rank", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep hybrid Fourier + low-rank vocab bridges.")
    ...
    print(f"pom_order={args.pom_order}, ordering=token_frequency, warmup_steps={args.warmup_steps}, residual_scale={args.residual_scale}")
