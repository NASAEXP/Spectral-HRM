...
def _sla_attention_class():
    assert hasattr(layers, "SLAAttention"), "SLAAttention is not implemented yet."
    return layers.SLAAttention
...
def test_sla_attention_preserves_shape_and_backpropagates():
    SLAAttention = _sla_attention_class()
...
def test_sla_prefix_outputs_do_not_read_response_tokens():
    SLAAttention = _sla_attention_class()
...
def test_sla_causal_outputs_do_not_read_later_tokens():
    SLAAttention = _sla_attention_class()
...
def test_sla_head_competition_is_a_softmax_over_heads():
    SLAAttention = _sla_attention_class()
    attn = SLAAttention(
        hidden_size=16,
        head_dim=4,
    ...
    weights = attn._head_competition(logits)
...
def test_transformer_config_can_select_sla_token_mixer():
    config = TransformerConfig(
        max_seq_len=8,
        n_layers=1,
        hidden_size=16,
    ...
    assert isinstance(block.attn, _sla_attention_class())


def test_hrm_can_use_pom_l_level_and_sla_h_level():
    config = {
        "vocab_size": 260,
        "max_seq_len": 8,
        "n_layers": 2,
        ...
        "bp_max_steps": 2,
        "token_mixer": "pom",
        "H_override": {"token_mixer": "sla"},
    }

    model = HierarchicalReasoningModel(config)

    assert isinstance(model.L_level.core.layers[0].attn, _pom_attention_class())
    assert isinstance(model.H_level.core.layers[0].attn, _sla_attention_class())
